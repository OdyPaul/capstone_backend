...
// Resolve actual student profile collection name
...
const createVCRequest = asyncHandler(async (req, res) => {
  ...
  if (!type)   { res.status(400); throw new Error('Missing field: type'); }
  if (!purpose){ res.status(400); throw new Error('Missing field: purpose'); }
  ...
  if (!PURPOSES.includes(purpose)) {
    res.status(400); throw new Error(`Invalid purpose. Allowed: ${PURPOSES.join(' | ')}`);
  }

  // ensure user is verified and has studentId
...
/** PATCH /api/vc-requests/:id (admin) */
const reviewVCRequest = asyncHandler(async (req, res) => {
  const { status } = req.body;
  const valid = ['approved','rejected','issued'];
  if (!valid.includes(status)) { res.status(400); throw new Error(`Invalid status. Allowed: ${valid.join(', ')}`); }
  ...
  res.status(200).json({
    ...
    studentId: doc.studentId,
    type: doc.type,
    purpose: doc.purpose, // ✅ keep returning
    status: doc.status,
