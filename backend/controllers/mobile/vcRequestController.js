...
const getMyVCRequests = asyncHandler(async (req, res) => {
  const list = await VCRequest.find({ student: req.user._id })
    .select('_id student studentId type purpose status createdAt updatedAt')
    .sort({ createdAt: -1 })
    .lean();
...
const getAllVCRequests = asyncHandler(async (_req, res) => {
  // ✅ Use the same Students model you use elsewhere
  let STUDENT_COLLECTION = 'students';
  try {
    const Student = require('../../models/students/studentModel');
    if (Student?.collection?.name) STUDENT_COLLECTION = Student.collection.name; // usually 'students'
  } catch {}
  ...
  const rows = await VCRequest.aggregate([
    ...
    // ✅ Join to the actual students collection via studentId -> _id
    ...
    { $project: {
        ...
        // ✅ These match your students model fields used elsewhere
